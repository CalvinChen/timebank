...
public class HomepageAction {
	...
	public void setUser(User user) {this.user = user;}
		//--------------------viewRecords---------------------------
	private ArrayList<HelpRecord> list;
	public ArrayList<HelpRecord> getList() {return list;}
	public void setList(ArrayList<HelpRecord> list) {this.list = list;}
	
	//-----------------------viewOne-----------------------
	private int recordId;
	private HelpRecord one;
	...
	public HelpRecord getOne() {return one;}
	public void setOne(HelpRecord one) {this.one = one;}
	...
	private void getUserFromUserLogined() {
		...
		balance = bankInfoManager.getBalanceByUserId(user.getId());
	}
	...
	public String viewMessage(){
		...
		myMessages = messageManager.getOnesMessage(user.getId());
		getUnreadMsgCount();
		return "viewMessage";
	}
	/**
...
//		}
	...
	public String infoSubmit() {
		...
		if (result.equals(Values.SUCCESS)) {
			...
			userLogined = userManager.login(null, null);
	...
	public String picSubmit() {
		...
		if (picture != null) {
			...
			try {
				...
				user = (User) ActionContext.getContext().getSession().get("userLogined");
				getUserFromUserLogined();
				UserManagerImpl.deleteUserPicture(user.getUserInfo().getUserPhoto());
				boolean result = userManager.updatePhoto(user.getId(),	fileNameAll);
				if (result == true) {
					Tester.markHere("result == true");
					// 头像更新成功
					try {
	...
	public String viewRecords(){
		getUserFromUserLogined();
		list = helpRecordManager .readSomeByUid(user.getId());
		return "viewRecords";
	}
