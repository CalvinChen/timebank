...
//	}
	...
	public static String verifyAdminModifyUser(User user){
		...
		//用户名
		User oneDatabase = new UserManagerImpl().getByUsername(user.getLoginName());
		if(oneDatabase != null && oneDatabase.getId() != user.getId()){
			return "此用户名已被他人注册";
		}
		length = user.getLoginName().length();
