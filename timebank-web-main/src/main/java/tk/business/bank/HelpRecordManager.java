...
import tk.persistence.HelpRecordDao;
...
//			}
	...
	public String updateFBpartById(int requireBy, int userId ,int helpRecordId, double userScore, String userFeedback) {
		...
		//读出被评价者的银行资料
		BankInfo b = bankInfoDao.readOneByUserId(userId);
...
//			}
	...
	public String updateStatusById(int helpRecordId, int status, int score, String whyVerifyFailure, String verifier) {
		...
		//取出记录，用于看是否有链接记录
		HelpRecord one =  helpRecordDao.readOneById(helpRecordId);
...
//		}
	 ...
	 * @return 该用户所有的互助记录
	 */
	public ArrayList<HelpRecord> readSomeByUid(int userId) {
		ArrayList<HelpRecord> list = helpRecordDao.readSomeByUserId(userId);
		return list;
	}
	 ...
	 * @return 所有该状态的互助记录列表 
	 */
	public ArrayList<HelpRecord> readSomeByStatus(int status, int page) {
		ArrayList<HelpRecord> list = helpRecordDao.readSomeByStatus(status, page);
		return list;
	}
	 ...
	 * @return 该互助记录
	 */
	public HelpRecord readOneById(int helpRecordId) {
		//取出记录
		HelpRecord helpRecord = helpRecordDao .readOneById(helpRecordId);
		return helpRecord;
	}
	 ...
	 * @return 增加的结果
	 */
	public String createOne(HelpRecord one){
//		if(one.getLinkDepositId() != 0){
