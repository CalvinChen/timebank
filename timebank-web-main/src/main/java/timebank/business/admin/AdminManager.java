...
import timebank.persistence.AdminDao;
...
//	@Resource
	...
	public Admin login(Admin oneInput) {
		Admin oneDatabase = adminDao.readOne(oneInput.getUsername());
		if(oneDatabase == null || 
	...
	public String updateOne(Admin one) {
		Admin oldOne = adminDao.readOne(one.getAdminId());
...
//		one.setLinkedUserId(oldOne.getLinkedUserId());
	...
	public String promote(int uid) {
		...
		Admin admin = adminDao.readOne(user.getUsername());
		if(admin != null){
