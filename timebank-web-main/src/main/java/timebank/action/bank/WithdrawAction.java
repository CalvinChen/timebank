...
public class WithdrawAction {
	...
	private ArrayList<BankCoreBusiness> recordList;
	private ArrayList<BankCoreBusiness> topList;

	public ArrayList<BankCoreBusiness> getTopList() {
		return topList;
	}
	public void setTopList(ArrayList<BankCoreBusiness> topList) {
		this.topList = topList;
	}
	public ArrayList<BankCoreBusiness> getRecordList() {
		return recordList;
	}
	public void setRecordList(ArrayList<BankCoreBusiness> recordList) {
		this.recordList = recordList;
	}
	
	//-----------viewOne------------------------------------------
	private BankCoreBusiness depositRecord;
	private int myUserId;
	public BankCoreBusiness getDepositRecord() {
		return depositRecord;
	}
	public void setDepositRecord(BankCoreBusiness depositRecord) {
	...
	private int count;
	private int pages;
	private ArrayList<BankCoreBusiness> top2List;
	
	public ArrayList<BankCoreBusiness> getTop2List() {
		return top2List;
	}
	public void setTop2List(ArrayList<BankCoreBusiness> top2List) {
	...
	public String viewOne(){
		BankCoreBusiness one = depositManager .getOneDepositByDepositId(depositId);
		if(one != null){
			depositRecord = one;
			myUserId = ((User)ActionContext.getContext().getSession().get("userLogined")).getUserId();
			return "viewOne";
		}else{
	...
	public String askForHelp(){
		...
		if(depositRecord != null){
			...
			depositId = depositRecord.getBankCoreBusinessId();
		}else{
	...
	public String require(){
		...
		if(result.equals(Values.SUCCESS) == false){
			message = result;
			depositRecord = depositManager .getOneDepositByDepositId(depositRecord.getBankCoreBusinessId());
			return "viewOne";
		}
		
		//获得帮助者与求助者的ID
		int helpedUserId = ((User)ActionContext.getContext().getSession().get("userLogined")).getUserId();
		int toHelpUserId = depositRecord.getRequester().getUserId();
		
		BankCoreBusiness one = depositManager.getOneDepositByDepositId(depositRecord.getBankCoreBusinessId());
...
//		newRecord.setLinkWithdrawId(0);
	...
	public String submit(){
		...
		if(withdrawManager.addOneWithdraw(one, user.getUserId()).equals(Values.SUCCESS)){
			forward = "succ";
		}else{
